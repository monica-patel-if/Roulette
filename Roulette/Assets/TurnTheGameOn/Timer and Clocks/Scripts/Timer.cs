		...
		void Update () {
			...
			if (timerState == TimerState.Counting) {
				...
				if (timerType == TimerType.CountDown) {
                    ...
                    //}
                    try {
                        //if ((int)gameTime == msgSystem.ins.NomoreBets)
                        {
                            //msgSystem.ins.msgBoxPanel.SetActive(true);
                        }
                        if ((int)gameTime == msgSystem.ins.NomoreBets)
                        {
                           ...
                           msgSystem.ins.PopUpMsg.text = "NO MORE BETS !!!";
                            ...
                            //BettingRules.ins.EventSystem.SetActive(false);
                            UIManager.ins.InteractionUI.SetActive(true);
                          // timerState = TimerState.Disabled;
                        }
                       if ((int)gameTime <= 5)
                        {
                            timerTextDefault.color = Color.red;
                            msgSystem.ins.FillCounterImg.color = Color.red;
                        }

                        float fill = gameTime / startTime;
                        msgSystem.ins.FillCounterImg.fillAmount = fill;
                    }
                    catch { }
