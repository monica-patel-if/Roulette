...
public class UniWebView: MonoBehaviour {
    ...
    void Awake() {
        ...
        transform.GetComponent<RawImage>().enabled = false;
         listenerObject = new GameObject(id);
        ...
        listener.webView.backgroundColor = Color.black;
    ...
    /// configuration.
    /// 
    /// This only works for iOS and macOS Editor. It is using some "hidden" way of setting a config for WebKit. So it is
    /// possible that it stops working in a future version.
    /// 
    /// On Android, the CORS rule checking is ignored by system `WebView`.
    /// </summary>
